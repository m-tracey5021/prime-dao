...
func NewRequestManager[T schema.Identifiable](path, descriptor string, id uint64) (*DaoRequestManager[T], error) {

	dao, err := New[T](path, descriptor, id)
	...
	queue := queue.NewQueue(10, 5, 100, processor)

	return &DaoRequestManager[T]{queue}, nil
}

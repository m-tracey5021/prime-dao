...
func (manager *QueueRequestManager[T]) Execute(request IRequest[T]) IResult[T] {

	return manager.queue.ProcessSync(request)
}

func (manager *QueueRequestManager[T]) ExecuteAsync(requests ...IRequest[T]) {
...
func (manager *QueueRequestManager[T]) Results() []TSFQueueResult[T] {

	results := manager.queue.Stop()
